...
public class TimSoDuongDauTienTrongMang2Chieu {
    ...
    // Hàm tìm số nguyên dương đầu tiên xuất hiện
    public static void TimSoDuongDauTien(int a[][], int soCot, int soDong) {
        for (int i = 0; i < soDong; i++) {
            for (int j = 0; j < soCot; j++) {
                if (a[i][j] > 0) {
                    System.out.println("So duong dau tien trong ma tran la: " + a[i][j] + " [" + i + "][" + j + "]");
                    return;
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        TimSoDuongDauTien(a, soCot, soDong);
    }
}
